...
- **AOD Session Countdown:** the opt-in **Session Countdown** AOD style leaves interactive mode and AOD styles 0–2 unchanged, then replaces the ambient digital clock with dimmed, zero-padded remaining `hh`/`mm` rows and an uppercase session label: **PEAK** 06:00–11:00, **TRANSITION** 11:00–12:00, **TROUGH** 12:00–17:00, **PERSONAL** 17:00–22:00, and **SLEEP** 22:00–06:00. Its thin outer arc begins at `[HOUR_0_11] * 30 + [MINUTE] * 0.5` and ends at the current session boundary (for example, 14:30 is `02h`, `30m`, `TROUGH`, `75°→150°`; TRANSITION ends at `360°` so its clockwise sweep reaches 12:00). The arc is intentionally hidden during SLEEP for burn-in safety and because its selected non-SLEEP endpoint expression has no overnight branch. This resource-only WFF v2 implementation uses schema-valid `[HOUR_0_23]`, `floor()`, WFF `numberFormat("00", …)`, and `%s` Templates; the repository verifier evaluates the extracted countdown strings at every local minute against an independent session specification.
