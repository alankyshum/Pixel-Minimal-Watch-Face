...
- **AOD Session Countdown:** the opt-in **Session Countdown** AOD style leaves interactive mode and AOD styles 0–2 unchanged, then replaces the ambient clock with solid-white, zero-padded `hh`/`mm` rows at **158px**. Sessions are **PEAK** 06:00–11:00, **TRANSITION** 11:00–12:00, **TROUGH** 12:00–17:00, **PERSONAL** 17:00–22:00, and **SLEEP** 22:00–06:00. The 3px `r=215` arc begins at the hour-hand angle and ends at the active boundary. The marker’s opaque apex is at `r=217`, overlapping the arc’s `r=216.5` outer edge by 0.5px while retaining an 8px margin to the physical `r=225` face. In every non-SLEEP minute, 24px `HH:mm` uses a 24° path centered at `r=204` and trails the marker by a constant 20° counter-clockwise; 24px endpoint labels are at `r=195`, so radial and Euclidean separation prevent collision. At 158px, measured Nova Mono ink is 243×118px for `00h` and 250×117px for `00m`; their y=68 and y=226 rasters yield a 40.5px ink gap, within which the 26px session label has 18.5px clearance below the hour row and 3px above the minute row. The worst 24px endpoint, `17:00`, measures 63×19px. The verifier exhaustively evaluates all 960 non-SLEEP minutes for normalized constant-trail paths, no jumps, arc/marker junction overlap, endpoint wrapping, and current/endpoint/arc/center collision bounds. SLEEP has no arc furnishings. All Session Countdown ink is intentionally `#ffffff` at ambient alpha 255 for readability.
